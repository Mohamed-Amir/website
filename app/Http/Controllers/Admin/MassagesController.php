...
class MassagesController extends Controller
{
    ...
    {
        return view('Admin.Contact_form.index');
    }
    ...
    public function destroy($id,Request $request)
    {
        if ($request->type == 2) {
            $ids = explode(',', $id);
            $Contact_form = Contact_form::whereIn('id', $ids)->get();
            foreach($Contact_form as $row){
                $this->deleteRow($row);
            }
        } else {
            $Contact_form = Contact_form::find($id);
            $this->deleteRow($Contact_form);
        }
     ...
     * @param $cat
     */
    private function deleteRow($Contact_form){
        $Contact_form->delete();
    }
